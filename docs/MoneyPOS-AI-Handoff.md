...
**选择第五个共享 Entity 物理归属迁移切片。**

先完整阅读 `MoneyPOS-AD-2.1-Shared-Entity-Consumer-Inventory.md`、`MoneyPOS-AD-2.8-Fourth-Entity-Slice-Selection.md`，并重跑当前扫描。已固定的选择边界：
...
AD-2.10 完成后，按债务清单进入所选 Entity 的单独迁移切片。门禁仅阻止新引入的、已分类非所有者 Entity 契约；不能
把当前报告数量直接设为失败规则。
