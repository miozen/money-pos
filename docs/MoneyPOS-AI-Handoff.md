...
接力前已同步基线是 **`67ca28b feat(ad-2.53): migrate coupon rule entity`**；AD-2.54 已在本地完成并待提交/推送。此前的相邻架构提交见 `git log --oneline`。
...
**TRADE `OmsOrderDetail` Entity 物理归属迁移。**

先完整阅读实施台账、债务清单和 `MoneyPOS-AD-2.54-Twenty-Seventh-Entity-Slice-Selection.md`。将
`OmsOrderDetail` 迁入 TRADE 持久化边界，更新 Mapper、明细服务、结账、退款、查询、打印消费者及测试。

- 保持 `BaseEntity`、表/Flyway、明细所有快照字段、结账/库存/退款原子守卫、订单 DTO、打印与报表口径；不得改变路由、事务边界或业务公式。
- 新增 TRADE 本地 Mapper CRUD 回归，并保留结账、部分/全额退款、订单查询、打印、FIN/HOME 与会员排行回归。
- 以 AD-2.53 后扫描实测为基线：共享 Feature import 29、所有权登记 3、跨域桥 0、通配符路径 2；完成后预期登记 2，桥和通配符不得扩大。

## 后续编号顺序

AD-2.55 是唯一允许开始的迁移切片；门禁仅阻止新引入的、已分类非所有者 Entity 契约，不能把当前报告数量直接设为失败规则。
