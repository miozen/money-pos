...
### Completed: AD-7 Runtime Startup-Load and Redundancy Inventory/Cut Design

- Published `MoneyPOS-AD-7-Runtime-Startup-Load-and-Redundancy-Inventory.md` after read-only tracing of the Electron launcher, embedded MariaDB path, Spring startup entry, active runners/listeners/tasks, auto-configurations, front-end consumers and production source consumers. No source, dependency, profile, route, database/Flyway, transaction, task or hardware behavior changed.
- The inventory distinguishes cold-start phases rather than treating every Bean as a culprit. Embedded MariaDB and Spring/Flyway/Hikari are the principal unmeasured paths; HOME's seven-day compensation/current-day refresh is the only startup-triggered business aggregation and is a deferred-execution measurement candidate. Electron's fixed 1.5-second health retry is a window-readiness optimization candidate, not dead code.
- WebSocket customer display, backup, local asset storage, payment dictionary validation, security, tenant, Flyway and actuator health have concrete consumers and remain retained. Mail, local cache, timezone aspect and Swagger are only low-expected-gain candidates pending separate runtime/consumer verification; no static absence was treated as deletion authority.
- No trustworthy Windows cold/warm numbers were available: Windows packaged runtime remains ENV-1, and launching the development profile would write HOME snapshots. AD-7 therefore records a T0–T5 repeatable Windows measurement protocol rather than fabricating results.
- Next: AD-5 architecture-gate CI integration design; after its authorized implementation, any runtime removal/conditionalization needs a separately numbered, evidence-backed task.

### Completed: AD-5 CI Architecture-Gate Integration Design

- Published `MoneyPOS-AD-5-CI-Architecture-Gate-Design.md` after a read-only audit of the sole existing Actions workflow, scanner execution/fixture semantics, checked-in baselines, branch flow and database/secret boundaries. No workflow, repository setting, Maven/Node build, scanner, baseline or production source changed.
- The existing Windows EXE job is release-only: it triggers on `main`, skips Maven tests and downloads/builds JRE, MariaDB, frontend and installer artifacts. The design therefore selects a separate `ubuntu-latest` static job for `dev` push, PRs targeting `dev`/`main` and manual dispatch; it runs only explicit Bash fixture and additions-only scan commands with `contents: read` permission.
- The design preserves the scanner's current failure contract. It fails on new structural/Entity findings and tool/fixture errors, never on default report output or historical report metrics; the non-executable fixture file is invoked through `bash`. It deliberately does not add database credentials, `pull_request_target`, `continue-on-error`, release changes or automatic baseline governance.
- AD-5.1 is now the smallest implementation task. It requires a single new workflow file plus actual GitHub Actions verification on dev, PR and a discardable intentional-violation branch; branch protection remains an administrator decision after that proof.

### Completed: AD-5.1 CI Architecture-Gate Workflow Implementation

- Added `.github/workflows/architecture-gate.yml` as the sole implementation artifact. It runs on `dev` push, PRs targeting `dev`/`main` and manual dispatch; grants only `contents: read`; installs `ripgrep`; then explicitly runs the scanner fixtures and `architecture-scan.sh --check-new`. It neither changes the main-only Windows EXE workflow nor reads database credentials, starts application services or builds release artifacts.
- The first dev run exposed a real hosted-runner prerequisite: `rg` was absent and the verify step failed with exit 127. Follow-up commit `47fb359` added explicit APT installation; its dev run passed all four steps.
- Verified failure propagation with discardable branch `codex/ad5-1-negative-gate` / commit `ad7f933`: a sole Controller→Mapper import caused the PR gate to exit 1 and name `Ad51GateViolationController.java`. The PR was not merged; both remote and local test branches were deleted, and `dev...origin/dev` was restored.
- Branch protection required status checks remain an administrator-owned optional configuration. Next: ENV-1 Windows packaged embedded-MariaDB acceptance and AD-7 T0–T5 startup baseline collection; no runtime pruning is yet authorized.

### Completed: AD-6 Java 17 Source-Baseline Assessment

- Published `MoneyPOS-AD-6-Java17-Source-Baseline-Assessment.md` after read-only inspection of root/module POMs, JDK/Maven environment, Windows release workflow, source API use, fat-JAR bytecode, framework namespace and test topology. No POM, source, workflow, dependency, database or package changed.
- The assessment approves Java 17 as the main reactor's minimum build/runtime baseline: the Windows release already embeds Temurin 17 and Spring Boot 2.7.18 officially supports Java 17. It explicitly excludes Spring Boot 3 and `javax`/`jakarta`; the main reactor has 65 `javax.*` imports and none under `jakarta.*`.
- Found a material compatibility discrepancy: the fat JAR emits Java 8 class files (major 52) while production code directly invokes Java 9 `List.of` / `Map.of`. Thus `source`/`target` 1.8 do not prove Java 8 runtime compatibility and the declared Java 8 baseline must not be retained as a supported contract.
- AD-6.1 is the next, separately authorized, smallest implementation: use an explicit `release=17` compiler contract and Java-minimum enforcement, then run isolated full regression, package, architecture gate and Windows EXE confirmation. It must not include business changes, Boot 3/Jakarta, SQLite, Maven splitting, runtime pruning or `xxl-job-admin`.

### Implemented Locally: AD-6.1 Java 17 Build Baseline

- Updated only the main reactor build governance: Java version is 17, Maven Compiler Plugin 3.13.0 compiles with `release=17`, and Maven Enforcer requires Java `[17,)`. The existing Windows workflow remains on its single Temurin 17 JRE; no Spring Boot, `javax`/`jakarta`, database, Flyway, Electron, SQLite, Maven-module or business change was made.
...
### Operational Decision: Freeze Startup Optimization and Record Receipt Acceptance

- The user confirmed repeated packaged starts of roughly 15 seconds are acceptable and explicitly chose not to continue startup optimization. ENV-1's unexecuted first-init, instance-reuse, port-conflict and shutdown measurements, and every AD-7 candidate (including HOME deferral and Electron health polling), are frozen rather than pending.
- The receipt printer was manually tested successfully. This closes the receipt-printing part of ENV-2; cash-drawer validation remains conditional on the store actually using that hardware. No source, package, configuration or hardware behavior changed for this record.
- `MoneyPOS-Current-Architecture-and-Business-Scenarios.md` is the current source-backed orientation document. Historical design/selection files retain their original contemporaneous “next task” wording and must not be mistaken for active backlog.
