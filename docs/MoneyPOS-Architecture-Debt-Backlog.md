...
| AD-2.29 | SYS 打印配置 Entity 物理归属迁移 | 待实施 | 将 `SysPrintConfig` 移至 SYS 持久化实体包，更新 Mapper、服务接口/实现、Controller 和打印服务，并补 Mapper 与配置接口回归。 | 保持显式表名、BaseEntity/INPUT ID、固定 ID=1、种子记录、路由/JSON/异常、表/Flyway 与硬件行为不变；不新增多配置或删除语义。 |
...
1. **AD-2.29**：迁移 `SysPrintConfig` 到 SYS 持久化实体包，并补 Mapper 与打印配置接口回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.29：迁移 `SysPrintConfig` 到 SYS 持久化实体包，并补 Mapper 与打印配置接口回归。**
