...
| AD-2.31 | GMS 库存单据主表 Entity 物理归属迁移 | 待实施 | 将 `GmsInventoryDoc` 移至 GMS 持久化实体包，更新 Mapper、四个 GMS 消费者和受影响测试，并补 Mapper 与库存/结账/财务快照回归。 | 不移动 Mapper/Controller/DTO 或跨域契约；不改显式表名、BaseEntity/ID 行为、库存事务、快照口径、表/Flyway/唯一键。 |
...
1. **AD-2.31**：迁移 GMS `GmsInventoryDoc` 到持久化实体包，并补 Mapper、库存单据、结账/退款与财务快照回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.31：GMS 库存单据主表 Entity 物理归属迁移。**
