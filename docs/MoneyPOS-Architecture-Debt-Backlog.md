...
| AD-1.1 | `DecisionEngine` 写入行为盘点与特征化 | **待实施** | 盘点 `compensateSnapshots(7)`、`generateDailySnapshot(today)`、查询后 update/insert、告警均值读取的调用时序及竞争窗口。 | 增加受控并发/重复调用特征测试；记录是否需要唯一约束、锁或显式命令入口，暂不改变数据库或 HTTP 路由。 |
| AD-1.2 | HOME 快照命令模型设计 | 待设计 | 在 AD-1.1 证据上确定幂等键、并发策略、补偿调度位置与“读请求是否可写”的兼容迁移方案。 | 需明确是否允许表约束/Flyway、任务调度与页面刷新语义变更；这是用户授权点。 |
| AD-1.3 | HOME 写入边界实施与验收 | 受 AD-1.2 约束 | 仅按获批方案实施 HOME 内部命令/读分离，保留快照数值、告警字段和历史补偿能力。 | HOME 控制器、快照补偿、并发/重复请求、全量测试与架构门禁。 |
...
1. **AD-1.1**：先量化 HOME 写入时机与并发风险，再决定是否、如何修改 `DecisionEngine`。
...
5. AD-5 与 AD-6 分别作为工程治理和平台升级任务，不与业务架构迁移混做。

## 当前下一最小任务

**AD-1.1：盘点并特征化 HOME `DecisionEngine` 的日快照写入时序、幂等性与并发窗口。**
