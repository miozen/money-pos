...
| AD-2.9 | GMS 库存单明细 Entity 物理归属迁移 | 待实施 | 将 `GmsInventoryOrderDetail` 移到 GMS 持久化 entity 包，更新 GMS 服务/Mapper 导入并增加 CRUD 回归。 | 不移动主单、Mapper 或扫描根，不改隐式表名/Flyway/API；Feature 文件扫描预计降至 72。 |
...
1. **AD-2.9**：迁移 GMS 库存单明细 Entity 并增加 CRUD 回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.9：迁移 GMS 库存单明细 Entity 并增加 CRUD 回归。**
