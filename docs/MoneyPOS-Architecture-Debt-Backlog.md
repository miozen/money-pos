...
| **AD-2** | 共享 Entity 物理归属收敛 | **实施中** | 将目前逻辑归属已明确、但仍放在共享 `com.money.entity` 的类型逐个迁到所有者持久化边界或以所有者 DTO 替代跨域暴露。 | 当前 74 个 Feature 文件 import 共享 Entity 只是报告指标，不可按数量机械迁移。 |
...
| AD-2.5 | GMS 周转预警快照 Entity 物理归属迁移 | 待实施 | 将 `GmsTurnoverWarningSnapshot` 移到 GMS 持久化 entity 包，更新 GMS 服务/Mapper 导入并增加周转快照回归。 | 不改周转算法、路由、Excel、SYS 策略读取、表/Flyway、唯一键或吞没异常语义。 |
...
1. **AD-2.5**：迁移 GMS 周转预警快照 Entity 并增加回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.5：迁移 GMS 周转预警快照 Entity 并增加回归。**
