...
| **AD-7** | 运行时启动负担与无效能力盘点/裁剪设计 | **待设计** | 以启动画像、Bean/自动配置清单、前后端调用证据和运行期特征测试识别可删除、可按配置关闭与必须保留的能力；本任务只盘点和设计，不删除源码或关闭功能。 | 优先量化 HOME 启动快照补偿/刷新；不得把“暂未发现静态调用”直接等同于无效代码，需核对路由、WebSocket、定时任务、硬件与运维功能。裁剪实施须在 CI 门禁落地后另行授权。 |
...
1. **AD-7**：先量化启动时间与启动期功能，完成无效能力盘点和裁剪设计；不在该任务中删除代码。
2. **AD-5**：评估并设计架构门禁接入 CI；设计获批后，先独立实施门禁，为后续裁剪建立回归安全网。
3. **AD-7 后续裁剪实施**：仅针对 AD-7 已证明无消费者、无条件加载需求且已具备回归覆盖的最小候选，另行编号和授权。
4. **AD-6**：Java 17 源码基线升级评估，需独立的平台升级授权。
5. **ENV-1**：Windows 打包版嵌入式 MariaDB 验收；应复用 AD-7 的启动测量口径，但不能以源码测试替代。
6. **ENV-2**：小票/钱箱硬件验收，保持独立于启动裁剪。

SQLite 已完成只读可行性评估，但用户当前决定不进入迁移计划，故不登记为活跃债务或当前前置条件。

## 当前下一最小任务

**AD-7：运行时启动负担与无效能力盘点/裁剪设计。**

先阅读实施台账、债务清单、`MoneyPOS-AD-4-Maven-Physical-Modularization-Reassessment.md` 和运行时能力资料；
建立可重复的冷/热启动分段测量、启动期 Bean/自动配置与主动任务清单、候选能力调用证据矩阵，并将候选分为
“保留”“可配置关闭”“待裁剪验证”。不得在盘点任务中移除依赖、关闭 Bean、改变路由、数据库/Flyway、交易事务
或硬件/备份行为。已知优先核对项为 HOME 启动快照补偿/刷新、支付字典校验、定时备份、WebSocket、邮件、
Actuator、租户/国际化/时区及文件存储自动配置。
