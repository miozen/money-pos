...
| AD-2.41 | GMS 库存流水 Entity 物理归属迁移 | 待实施 | 将 `GmsStockLog` 移至 GMS 持久化实体包并补 Mapper、库存命令/查询/分析与 Controller 回归。 | 不改库存事务、流水公式、路由/JSON、表/Flyway 或跨域契约。 |
...
1. **AD-2.41**：迁移 GMS `GmsStockLog` 到持久化实体包。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.41：GMS 库存流水 Entity 物理归属迁移。**
