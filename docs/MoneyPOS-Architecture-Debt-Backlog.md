...
| AD-2.25 | SYS 省市区字典 Entity 物理归属迁移 | 待实施 | 将已验证的只读 `Provinces` 移至 SYS 持久化实体包，更新 Mapper、服务及测试导入。 | 不移动 Mapper/Controller/扫描根，不改显式八列查询、Flyway、tenant-ignore、路由或缓存；不补写入型 CRUD。 |
...
1. **AD-2.25**：迁移已验证的 `Provinces` 到 SYS 持久化实体包。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.25：迁移已验证的 `Provinces` 到 SYS 持久化实体包。**
