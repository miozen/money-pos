...
| AD-2.23 | GMS 库存单主表 Entity 物理归属迁移 | 待实施 | 将 `GmsInventoryOrder` 移到 GMS 持久化实体包，更新 Mapper、服务接口/实现，并补 Mapper 与库存单主流程回归。 | 保持 `ASSIGN_ID`、隐式表名、本域 `IService`、库存/成本/流水语义、表/Flyway/API/事务不变。 |
...
1. **AD-2.23**：迁移 `GmsInventoryOrder` 到 GMS 持久化实体包，并补 Mapper 与库存单主流程回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.23：迁移 `GmsInventoryOrder` 到 GMS 持久化实体包，并补 Mapper 与库存单主流程回归。**
