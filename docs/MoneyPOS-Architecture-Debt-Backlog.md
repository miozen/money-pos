...
| AD-2.8 | 第四个共享 Entity 物理归属迁移切片选择 | 待设计 | 重新盘点剩余共享 Entity 的所有者、消费者与可回归持久化语义，选择恰好一个最小迁移目标。 | 不在选择任务中移动 Entity；不得以扩展 additions-only 基线代替契约治理。 |
