...
| AD-2.13 | UMS 会员资金流水 Entity 物理归属迁移 | 待实施 | 将 `GmsMemberTransaction` 移到 UMS 持久化 entity 包，更新遗留 Mapper 泛型导入并增加 CRUD 回归。 | 不移动 Mapper 或服务，不改隐式表名/Flyway/API；所有权登记预计降至 23。 |
...
1. **AD-2.13**：迁移 UMS 会员资金流水 Entity 并增加 Mapper CRUD 回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.13：迁移 UMS 会员资金流水 Entity 并增加 Mapper CRUD 回归。**
