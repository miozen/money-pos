...
| AD-2.6 | 第三个共享 Entity 物理归属切片选择 | 待设计 | 重新确认剩余单一所有者候选的完整消费面、可回归行为与跨域契约风险，并只选择一个最小迁移切片。 | 不在选择任务中移动 Entity；不能以扩展 AD-2.3.1 基线代替契约治理。 |
