...
| --- | --- | --- | --- | --- |
| **AD-1** | HOME `DecisionEngine` 写入边界治理 | **待设计** | 将 `OmsDailySummary` 的补偿、当日重算、幂等和告警读取明确为 HOME 的命令/读模型责任；不把 HOME 快照写入交给 TRADE。 | 先固化 GET 重试、并发调用、补偿与当天覆盖行为；未经设计不得改变 `/home/count` 当前触发写入的兼容语义。 |
...
| AD-1.3 | HOME 写入边界实施与验收 | **待实施（分阶段）** | 先实施不改变 GET 时机的 AD-1.3a 命令提取与原子写入；AD-1.3b 仅在用户选择定时或事件刷新策略后去除 GET 写。 | HOME 控制器、快照补偿、并发/重复请求、全量测试与架构门禁。 |
...
1. **AD-1.3a**：实施 HOME 命令/查询分离及原子日快照写入，保持 GET 触发不变。
...
5. AD-1.3b、AD-5 与 AD-6 分别需要刷新策略、工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-1.3a：实施 HOME `DecisionEngine` 命令/查询分离与原子日快照写入，保持 GET 触发不变。**
