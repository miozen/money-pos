...
1. **ENV-1**：补齐 Windows 打包版嵌入式 MariaDB 的首次初始化、实例复用、端口冲突与关闭流程；既有数据启动、health、收银窗口及约 15 秒启动基线已验证。
2. **AD-7 后续裁剪实施**：仅在 ENV-1 分段测量确认候选收益后，针对已证明无消费者/可条件化且具备 CI 回归保护的最小候选另行编号和授权。
3. **ENV-2**：小票/钱箱硬件验收，保持独立于启动裁剪。

SQLite 已完成只读可行性评估，但用户当前决定不进入迁移计划，故不登记为活跃债务或当前前置条件。

## 当前下一最小任务

**ENV-1：Windows 打包版嵌入式 MariaDB 的剩余环境验收。**

既有数据启动、health 和收银窗口已通过；下一步仅补首次初始化、实例复用、端口冲突和关闭流程，并按 AD-7
的 T0–T5 口径分段记录。不得夹带数据库替换、Boot 3/Jakarta、XXL、Maven 拆分或启动裁剪。
