...
| AD-2.53 | UMS 满减券规则 Entity 物理归属迁移 | 待实施 | 先以 UMS 提供的 Entity-free 规则管理命令/查询契约替换遗留 TRADE 服务与 `/pos/couponRule` 的 Entity 暴露，再迁移 `PosCouponRule`。 | 保持管理端路由/JSON、分页、卡包、结账优惠、表/Flyway 与事务语义；预期登记册降至 3、桥降至 0。 |
...
1. **AD-2.53**：UMS `PosCouponRule` Entity 物理归属迁移。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.53：UMS 满减券规则 Entity 物理归属迁移。**
