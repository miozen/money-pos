...
| AD-2.19 | GMS 商品套餐明细 Entity 物理归属迁移 | 待实施 | 将 `GmsGoodsCombo` 移至 GMS 持久化 entity 包，更新四个 GMS 消费者/遗留 Mapper/测试导入，并补 Mapper CRUD 与套餐库存回归。 | 不移动 Mapper 或服务，不改 `ASSIGN_ID`、隐式表名、套餐库存命令、表/Flyway/API；所有权登记预计降至 20。 |
...
1. **AD-2.19**：迁移 GMS `GmsGoodsCombo` 并补 Mapper 与套餐库存回归。
2. 依赖 AD-2 结果实施门禁；随后才讨论 AD-4 的物理模块化。
3. AD-5 与 AD-6 分别需要工程治理和平台升级的独立授权。

## 当前下一最小任务

**AD-2.19：迁移 `GmsGoodsCombo` 到 GMS 持久化实体包，并补 Mapper 与套餐库存回归。**
