...
| AD-2.54 | 第二十七个共享 Entity 物理归属迁移切片选择 | 待实施 | 重新盘点余下 `OmsOrder`、`OmsOrderDetail` 与 `UmsMember`，选择唯一的下一最小迁移切片。 | 选择任务不得移动 Entity 或扩大当前两个通配符路径。 |
...
1. **AD-2.54**：重新盘点剩余三个共享 Entity，并选择唯一下一迁移切片。
