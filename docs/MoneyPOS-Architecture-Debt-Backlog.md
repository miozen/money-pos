...
| **AD-6.1** | 主 reactor Java 17 构建基线实施 | **已关闭** | 已以 `release=17`、受管编译器与最低 JDK 约束统一主 reactor；完整回归、打包、门禁及 Windows EXE 既有数据安装验收通过。 | 安装目录 `D:\30\_Data\WANXIANG-POS\vana-pos`、数据目录 `D:\WanXiang\POS-Data`；health 为 UP，收银窗口可用。 |
| **AD-7** | 运行时启动负担与无效能力盘点/裁剪设计 | **已关闭** | 已完成启动入口、主动任务、自动配置与前后端消费者盘点，建立 Windows 分段测量协议及候选分类；未删除源码或关闭功能。 | 用户已于 2026-09-22 决定接受约 15 秒启动表现并冻结后续测量/裁剪；不得将 HOME 或 Electron 候选作为当前任务。 |

## 环境验收待办（不是源码架构改造）

| 编号 | 事项 | 状态 | 完成条件 |
| --- | --- | --- | --- |
| ENV-1 | Windows 打包版嵌入式 MariaDB | **冻结** | 既有数据、health、收银窗口与约 15 秒重复启动已确认；用户决定不再测首次初始化、实例复用、端口冲突或关闭流程，除非日后明确恢复。 |
| ENV-2 | 小票/钱箱硬件 | **部分完成** | 收银小票打印已于 2026-09-22 实机通过。钱箱若在门店启用，仍需一次独立实机确认；退款小票目前没有产品入口，须另立功能需求。 |

## 推荐执行顺序

1. **无当前获准的软件改造任务**：架构收敛、Java 17、CI 门禁、订单状态兼容和收银小票均已闭环。
2. **ENV-2 钱箱确认**：仅当门店实际使用钱箱时执行一次实机确认；不与代码改造绑定。
3. **启动优化冻结**：不进行 ENV-1 剩余测量或 AD-7 裁剪，除非用户明确重新授权。
...
如门店实际启用钱箱，可执行 ENV-2 的一次实机确认。启动优化、SQLite、Maven 拆分、Boot 3/Jakarta 和无证据的代码裁剪均冻结；任何新改造须先单独盘点、编号并获授权。
