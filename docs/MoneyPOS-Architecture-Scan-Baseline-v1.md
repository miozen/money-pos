...
| Feature 导入共享 `com.money.entity` | 73 个文件 | 默认仍只报告总量；`scripts/architecture-baseline/shared-entity-owners.tsv` 记录 26 个 Entity 所有者，`shared-entity-cross-domain-baseline.tsv` 精确登记 6 条既有跨域桥，`shared-entity-wildcard-baseline.tsv` 记录 3 个既有通配符路径。 |
