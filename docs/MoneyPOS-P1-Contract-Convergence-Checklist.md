...
- [ ] P1.6 复核所有 P1 调用面、更新 Entity 归属表和架构扫描策略；P1.6.1 已完成调用面复核与“新增跨 Feature 实现依赖”门禁，详见 `MoneyPOS-P1.6-Call-Surface-Review.md`。库存写侧、结算试算和会员档案/券展示的剩余兼容调用按该文档拆分后，才可关闭 P1.6。仅在无循环、独立编译收益和运行时装配验证均满足时，重新评估 Maven 物理拆分。
...
下一最小任务是 **P1.6.2：为 P1.4c 库存写侧发布窄命令契约设计并迁移其 GMS 写入实现**。
