...
- [ ] P1.6 复核所有 P1 调用面、更新 Entity 归属表和架构扫描策略；P1.6.1 已完成调用面复核与“新增跨 Feature 实现依赖”门禁，P1.6.2 已完成库存写侧命令迁移，P1.6.3 已完成结算试算会员权益窄查询契约，详见 `MoneyPOS-P1.6-Call-Surface-Review.md`。POS 会员展示和 UMS 会员档案/券展示的剩余兼容调用按该文档拆分后，才可关闭 P1.6。仅在无循环、独立编译收益和运行时装配验证均满足时，重新评估 Maven 物理拆分。
...
下一最小任务是 **P1.6.4：迁移 TRADE POS 会员展示所需的会员权益与品牌名称窄查询契约**。
