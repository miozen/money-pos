...
- [x] P1.6 已完成所有 P1 调用面复核、Entity 归属复核和架构扫描策略更新：P1.6.1 的“新增跨 Feature 实现依赖”门禁、P1.6.2 库存写命令、P1.6.3 结算试算会员权益、P1.6.4 POS 会员展示、P1.6.5 UMS 会员档案/Excel 品牌选择契约均已闭环，详见 `MoneyPOS-P1.6-Call-Surface-Review.md`。P1 契约收敛已完成；Maven 物理拆分仍须在独立评估循环、独立编译收益和运行时装配后决定。
...
P1 已完成。下一最小任务应另行立项为 **P2：为 FIN/HOME 报表读模型盘点 Entity 与跨 Feature 调用，并按场景快照逐步收敛**。
