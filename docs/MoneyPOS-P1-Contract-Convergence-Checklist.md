...
- [x] P1.0 已建立契约收敛清单：以阶段 4 Entity 归属表和阶段 5 模块拆分决策为输入；初始 65 个、P1.6 复核时 68 个共享 Entity 导入仅作追踪，不将“减少数量”当作单独目标。
...
- [x] P1.4 迁移商品/价格与库存边界：
  ...
  - [x] P1.4c 库存写侧：新增 API 中立的销售扣库存/退款回库命令及 GMS 处理器；TRADE 的 `GoodsStockFacade` 只转换现有行 DTO，`PosInventoryActionService` 已删除。GMS 继续在既有结账/退款外层事务中完成商品、套餐、库存流水和库存单据写入。
- [x] P1.5 会员资产结算、退款命令：详见 `MoneyPOS-P1.5-Member-Asset-Command-Checklist.md`。
  - [x] P1.5.1 已完成写侧盘点与特征基线。
  - [x] P1.5.2 已补齐部分退款余额与券不足回滚特征测试；真实并发竞争留待 P1.5.6。
  - [x] P1.5.3 已完成结算命令设计。
  - [x] P1.5.4 已完成退款命令设计。
  - [x] P1.5.5 已实现结算/退款命令并迁移资产边界：TRADE 的 `MemberAssetFacade` 只适配既有结账/退款结果为 API 命令；UMS 拥有会员资产、满减券和余额写入，`PosAssetActionService` 已删除。
  - [x] P1.5.6 已完成：阶段 0 结账回归新增真实并发抢券、FIFO 和同请求重试覆盖；全量测试、打包和架构门禁通过。
- [ ] P1.6 复核所有 P1 调用面、更新 Entity 归属表和架构扫描策略；P1.6.1 已完成调用面复核与“新增跨 Feature 实现依赖”门禁，P1.6.2 已完成库存写侧命令迁移，P1.6.3 已完成结算试算会员权益窄查询契约，P1.6.4 已完成 POS 会员权益与品牌名称展示契约，详见 `MoneyPOS-P1.6-Call-Surface-Review.md`。UMS 会员档案/模板品牌展示的剩余兼容调用按该文档拆分后，才可关闭 P1.6。仅在无循环、独立编译收益和运行时装配验证均满足时，重新评估 Maven 物理拆分。
...
下一最小任务是 **P1.6.5：迁移 UMS 会员档案、模板和资产导出的品牌选择窄查询契约**。
