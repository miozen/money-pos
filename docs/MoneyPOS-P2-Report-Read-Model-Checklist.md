...
- [ ] P2.4.4.1 迁移 FIN 绩效报表与汇总卡片的 TRADE 周期经营原子指标：保留日/周/月分组、金融有效状态、闭区间和 FIN 展示计算。
- [ ] P2.4.4.2～P2.4.4.5 分别迁移销售看板、客流、品类/单品趋势与利润审计：按入口保持 GMS 显示档案、SYS 策略、分页和默认范围口径。
- [ ] P2.4.5 迁移瀑布流：分别收敛 TRADE/GMS 输入，FIN 仅组装既有公式；不把不同财务口径强行合并。
