...
- [x] 2.6.3 已完成网页端手工回归：POS 收银、全单/部分退款、订单查询、财务报表和首页看板均通过。收银小票打印已于 2026-09-22 完成实机验证；钱箱如启用仍按 ENV-2 独立确认。
- [x] 2.6.4 已更新实施台账：阶段 2 前置条件及已实施的打印验收均已满足；不以未启用的钱箱或尚不存在的退款小票入口阻塞阶段结论。
