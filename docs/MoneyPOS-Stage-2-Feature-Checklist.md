...
- [x] 2.4.5 已完成商品目录与价格/库存调用面盘点；仅迁移无外部调用的品牌定价策略控制器。商品、分类、套餐、价格、库存与库存日志类型因仍被 TRADE、HOME、UMS 或共享 Mapper 调用，暂留兼容包。
- [ ] 2.4.6 运行商品、库存、POS 结算与退款回归。
...
- [ ] 2.5.3 已完成会员流水、会员资产与充值两个 UMS 子域：控制器和服务归入 `feature.ums`，共享 Mapper 因被 TRADE/FIN/HOME/导入等调用而保留兼容包；会员档案、导入与 POS 兼容入口仍待逐子域迁移和独立提交。
